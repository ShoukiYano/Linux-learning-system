      ...
      <div className="p-8 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold">コマンド辞典エディタ</h1>
          </div>
          <button
            onClick={() => {
        ...
        {showForm && (
          <div className="bg-slate-800 rounded-2xl p-8 border border-slate-700 mb-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold">{editingId ? 'コマンド編集' : 'コマンド追加'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold mb-2">コマンド名</label>
                  <input
                    type="text"
                    value={formData.name}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 px-3 text-white focus:outline-none focus:border-primary-500 h-20"
                  required
                />
              </div>

              <div>
                <label className="block text-sm font-bold mb-2">使用方法</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-bold mb-2">使用例</label>
                <input
                  type="text"
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="p-6 border-b border-slate-700">
            <h2 className="text-xl font-bold">コマンド一覧（{commands.length}件）</h2>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-900">
                <tr>
                  <th className="px-6 py-3 text-left text-sm font-bold">コマンド</th>
              ...
              <tbody>
                {commands.map((cmd, idx) => (
                  <tr key={cmd.id} className={idx % 2 === 0 ? 'bg-slate-800/50' : ''}>
                    <td className="px-6 py-4 font-mono font-bold text-primary-400">{cmd.name}</td>
                    <td className="px-6 py-4 text-sm line-clamp-2">{cmd.description}</td>
                    <td className="px-6 py-4">{cmd.category}</td>
