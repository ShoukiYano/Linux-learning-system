      ...
      <div className="p-8 max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <BookOpenCheck size={32} className="text-primary-600 dark:text-primary-400" />
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">ヘルプ記事管理</h1>
          </div>
          {!showForm && (
            <button
        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 mb-8 shadow-lg">
            <h2 className="text-2xl font-bold mb-6 text-slate-900 dark:text-white">{editingArticle ? '記事を編集' : '新しい記事を作成'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-200">本文 *</label>
                <textarea
                  value={formData.content}
                  onChange={(e) => setFormData({ ...formData, content: e.target.value })}
                  className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-3 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary-500 font-mono h-48 shadow-sm dark:shadow-none"
              ...
              <div className="flex items-center gap-3 text-slate-700 dark:text-slate-200">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-primary-600"
                />
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm dark:shadow-none">
          <div className="p-6 border-b border-slate-200 dark:border-slate-700">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">記事一覧（{articles.length}件）</h2>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 dark:bg-slate-900">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-sm font-bold text-slate-500 dark:text-slate-400">ステータス</th>
                  <th className="px-6 py-3 text-left text-sm font-bold text-slate-500 dark:text-slate-400">作成日</th>
                  <th className="px-6 py-3 text-left text-sm font-bold text-slate-500 dark:text-slate-400">操作</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {articles.map((article, idx) => (
                  <tr key={article.id} className={article.id === editingArticle?.id ? 'bg-primary-50 dark:bg-primary-900/10' : (idx % 2 === 0 ? 'bg-white dark:bg-slate-800/50' : 'bg-slate-50/30 dark:bg-slate-800/20')}>
